...
- `# type: ignore` suppresses diagnostics for **both** mypy and pyright. `# pyright: ignore[rule]` suppresses only pyright. When mypy no longer needs a suppression but pyright still does, replace `# type: ignore` with `# pyright: ignore[rule]`. When both tools need suppression, keep `# type: ignore`.
