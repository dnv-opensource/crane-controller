...
class SimpleTestEnv(gym.Env[tuple[int, ...], int]):
    ...
    Args:
        ...
        reward_fac (tuple[float,float])=(1.0,1.0): the reward factors on positional and speed-related observations
        reward_limit (float)=None: The reward at which the episode is terminated (success)
        dt (float)=1.0: 
        pos0  (float)=0.0: initial position. Can be changed by initialize()
        speed0 (float)=0.0: initial speed. Can be changed by initialize()
        pos1 (float)=10.0: position goal. Can be changed by initialize()
        speed (float)=0.0: speed goal. Can be changed by initialize()
        seed (int): random seed value
        render_mode (str): render mode: 'none', 'plot'
    """
    def __init__(self,
                 acc:float=1.0,
        ...
        self.observation_space = spaces.MultiDiscrete(
            nvec = np.array([len(self.pos_range)+1, len(self.speed_range)+1],np.int16),
            dtype = np.int16,
            start = np.array( [-self.pos_range[0], -self.speed_range[0]], np.int16))
        self.action_space = spaces.Discrete( n=3, start=0, dtype=np.uint16)
        self.render_mode = render_mode
        self.steps = 0
        self.time = 0.0
        self.dt =dt # time step setting
        self.pos = self.pos0 = pos0 # initial position
        self.speed = self.speed0 = speed0 # initial speed
        self.pos1 = pos1 # goal position
        self.speed1 = speed1 # goal speed
        self.reward_limit = reward_limit if reward_limit is not None else self.calc_reward(self.pos1,self.speed1)
        print("LIMIT", self.reward_limit)
    ...
    def initialize(self,
                   ...
                   speed1: float,
                   pos0: float=0.0,
                   speed0: float=0.0,)->None:
        """Initialize the environment before a run (positions and speeds).

        Args:
            pos1 (int): Goal position as int
            speed1 (int): Goal speed as int
            pos0 (int)=0.0: Start position as int
            speed0 (int)=0.0: start speed as int
        """
    ...
    def calc_reward(self, pos:float, speed:float):
        """Calculate the reward based on internal information, position and speed."""
        reward = (self.reward_fac[0]* (2*abs(self.pos1-self.pos0) - abs(self.pos1 - pos)) +
                  self.reward_fac[1]* (2*abs(self.speed1-self.speed0) -  abs(self.speed1 - speed)))
        return reward
    ...
    def _get_obs(self, acc: float = 0.0) -> tuple[np.ndarray | tuple[int, ...], float, int]:
        ...
        truncate = not ((self.pos_range[0] < self.pos < self.pos_range[-1]) and
                        (self.speed_range[0] < self.speed < self.speed_range[-1]) and
                        reward > 0)
    ...
    def get_parameters(self) -> dict[str, Any]:
        ...
        return {
            "min_pos": self.pos_range[0],
            "max_pos": self.pos_range[1],
