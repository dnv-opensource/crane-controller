...
class CartPoleEnv(gym.Env):  # [np.ndarray, int | np.ndarray]):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        self.reset(seed=seed)  # EIS: added together with seed parameter to control random numbers
    ...
    def render(self):
        ...
        pole_coords = []
        for _coord in [(L, b), (L, t), (r, t), (r, b)]:
            coord = pygame.math.Vector2(_coord).rotate_rad(-x[2])
            _coord = (coord[0] + cartx, coord[1] + carty + axleoffset)
            pole_coords.append(_coord)
...
class CartPoleVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        self.screen_width = 600
        self.screen_height = 400
        self.screens: list[pygame.Surface] | None = None
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        self.state = self.np_random.uniform(low=self.low, high=self.high, size=(4, self.num_envs))
        self.steps_beyond_terminated = None
    ...
    def render(self):
        ...
        if self.state is None:
            raise ValueError("Cartpole's state is None, it probably hasn't be reset yet.")
        ...
        _res = [np.transpose(np.array(pygame.surfarray.pixels3d(screen)), axes=(1, 0, 2)) for screen in self.screens]
        return tuple(_res)

    def close(self):  # type: ignore[override] ## should work without additional arguments
