...
def test_levels(crane: Callable[..., Crane]) -> None:
    def check(val:float,expected:tuple[float,...]) -> None:
...
def test_discretization( crane: Callable[..., Crane], *, show: bool, discretization: dict[str:Any]) -> None:
    ...
    env = AntiPendulumEnv(
        ...
        render_mode="none",
        reward_limit=0.0,
        reward_fac=(0.01,0.01),
        discrete=discretization.copy(),
    )
    env.reset()
    agent = QLearningAgent(env)
    for a in range(len(discretization['angle'])-1):
        for s in range(len(discretization['speed'])-1):
            for c_p in range(len(discretization['c-pos'])-1):
                for c_s in range(len(discretization['c-speed'])-1):
                    action_sum = [0]*3
                    for angle, speed, c_pos, c_speed in itertools.product(
                        (discretization['angle'][a], discretization['angle'][a+1]),
                        (discretization['speed'][s], discretization['speed'][s+1]),
                        (discretization['c-pos'][c_p], discretization['c-pos'][c_p+1]),
                        (discretization['c-speed'][c_s], discretization['c-speed'][c_s+1])
                    ):
                    ...
                    if (max(action_sum)!=16 and
                        action_sum[0]>0  and
                        action_sum[2]>0 and
                        action_sum[0]==action_sum[2]
                    ):    
                        print(f"angle:{a}, speed:{s}, c_pos:{c_p}, c_speed:{c_s}: {action_sum}")
    

def test_state( crane: Callable[..., Crane], *, show: bool) -> None:
    """Set state and calculate reward."""
    def check_step(act:int,
    ...
    def get_state():
        ...
        state = (float(env.crane.position[0]),
    ...
    for s in range(len(actions)):
        ...
        if res[1] <= reward:
            print(f"step:{s}, actions:{actions[:s]}, obs:{res[0]}, reward:{float(res[1])}")
            break
        agent.update_q(obs0, actions[s], res[1], terminated=False, next_obs=res[0], prev_reward=reward)
        reward = float(res[1])
        reward_sum += reward
    print( f"reward:{reward}, avg:{reward_sum/s}")
    print(f"pos:{env.crane.position}, speed:{env.crane.velocity}, dir:{env.wire.direction}, v_w:{env.wire.cm_v}")
    for k,v in agent.q_values.items():
        print(k, v)
    #env.set_state(pos=18.0, speed=0.0, direction=0.0, w_speed=0.0)
    #print( env.step(2))
...
def test_state2( crane: Callable[..., Crane], *, show: bool) -> None:
    ...
    env = AntiPendulumEnv(
        ...
        render_mode="none",
        reward_limit=0.0,
        reward_fac=(0.01,0.000),
        discrete=AntiPendulumEnv.DISCRETE2.copy(),
    )
    env.reset()    
    agent = QLearningAgent(env)
    ...
    for a in range(3):
        ...
        env.step( a)
        #env.step(1)
        print(a, get_state()[1], env._get_obs())
...
def test_update_q_values(crane: Callable[..., Crane], *, show: bool) -> None:
    ...
    for i in range(1000):
        ...
        next_obs, _reward, terminated, truncated, _ = env.step(action)  # take action and observe result
        reward = float(_reward)
        agent.update_q(obs, action, reward, terminated=False, next_obs=next_obs, prev_reward=prev_reward)
        # Move to next state
        obs = next_obs
        nsteps += 1
        truncated = False
